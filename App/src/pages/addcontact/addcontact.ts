...
export class AddcontactPage {
  ...
  responseData : any;
  userDetails : any;
  userDataContact = {"fnameC": "","lnameC": "", "mnumberC": "", "addressC": "","lgt": "","ltt": "","user_id":""};
    ...
    return new Observable((sub: any) => {
      google.maps.event.addListener(autocomplete, 'place_changed', () => {
        ...
        if (!place.geometry) {
          ...
          this.userDataContact.ltt = place.geometry.location.lat();
          this.userDataContact.lgt = place.geometry.location.lng();
          sub.next(place.geometry.location);
          //sub.complete();
        }
      });
  ...
  initializeMap() {
    this.zone.run(() => {
      ...
      this.map = new google.maps.Map(mapEle, {
        zoom: 20,
        center: { lat: 31.6339793, lng: 74.87226420000002 },
  ...
  onsave(){
    ...
    this.userDataContact.user_id = this.userDetails.user_id;
    console.log(this.userDataContact);
    let loader = this.loadingCtrl.create({
      content: "Saving Data.."
    });  
    loader.present();
    if(this.userDataContact.fnameC && this.userDataContact.lnameC && this.userDataContact.mnumberC && this.userDataContact.addressC && this.userDataContact.lgt && this.userDataContact.ltt){
      //Api connections
    this.authService.postData(this.userDataContact, "addcontact").then((result) =>{
      loader.dismiss();
